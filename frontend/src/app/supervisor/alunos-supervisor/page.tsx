...
export default function TurmaPage() {
  ...
  const handleOpenFeedback = (student: any) => {
    setSelectedStudent(student)
    setShowFeedback(true)
  }

  return (
    <main className={`min-h-screen bg-white ${showFeedback ? "overflow-hidden" : ""}`}>
      <Header />
      <div className="container mx-auto pt-6">
        <div className="flex flex-col px-4">
          ...
          <div className="relative rounded-lg overflow-hidden my-4" style={{ backgroundColor: "#5a6e7e" }}>
            <Link
              href="/supervisor/conselho-supervisor"
                ...
                {students.map((student) => (
                  <div key={student.id} className="flex items-center justify-between py-4 px-4 bg-[#f9f9f9] rounded-md">
                    ...
                    <button
                      ...
                      className="inline-block bg-[#036ec5] hover:bg-[#025aa3] text-white font-medium py-2 px-6 rounded-md transition-colors"
                    >
                      Visualizar Feedback
                    </button>
                  </div>
      ...
      {showFeedback && selectedStudent && (
        <FeedbackPopup student={selectedStudent} onClose={() => setShowFeedback(false)} />
      )}
    </main>
  )
}
