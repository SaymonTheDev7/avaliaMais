...
interface PedagogicalItemProps {
  id: number;
  name: string;
  position: string;
  hours: number;
...
export function PedagogicalItem({
  id,
  name,
  position,
  hours,
  ...
  return (
    <div key={id} className="relative rounded-xl overflow-hidden shadow-md bg-[#003366] text-white h-[180px]">
      ...
      <div className="absolute top-10 left-4 transform -translate-y-1/2">
        <img
          src={photoUrl || "/placeholder.svg?height=60&width=60"}
          alt={name}
      ...
      <div className="p-4 mt-2">
        ...
        <button
          onClick={() => onRemovePedagogical(id)}
